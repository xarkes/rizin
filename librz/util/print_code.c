...
static void print_c_instructions(RzPrint *p, ut64 addr, const ut8 *buf, int len) {
	...
	while (!rz_print_is_interrupted() && i < len) {
		ut64 at = addr + i;
...
static void print_c_code(RzPrint *p, ut64 addr, const ut8 *buf, int len, int ws, int w) {
	...
	for (i = 0; !rz_print_is_interrupted() && i < len; i++) {
		...
		p->cb_printf(fmtstr, rz_read_ble(buf, p->big_endian, bits));
		if ((i + 1) < len) {
			p->cb_printf(",");
...
RZ_API void rz_print_code(RzPrint *p, ut64 addr, const ut8 *buf, int len, char lang) {
	...
	case 'V': // "pcV" // vlang.io
		...
		for (i = 1; !rz_print_is_interrupted() && i < len; i++) {
			rz_print_cursor(p, i, 1, 1);
			p->cb_printf("%d%s", buf[i], (i + 1 < len) ? ", " : "");
			rz_print_cursor(p, i, 1, 0);
			if ((i % 10) == 0) {
